...
{
    public class SlothFactCommandHandler
    {
        private readonly TwitchClient _client;
        private readonly Random _rng = new Random();

        public SlothFactCommandHandler(TwitchClient client)
        {
            _client = client;
        }
        ...
        private void GetSlothFact(OnChatCommandReceivedArgs e)
        {
            var i = _rng.Next(0, _slothFacts.Count);
            _client.SendMessage(e, _slothFacts[i]);
        }
